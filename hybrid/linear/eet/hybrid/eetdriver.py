...
#            times, rhos_site,rhos_eig = my_hybrid.propagate(rho_g, 0.0, t_final, dt)
                ...
                omegas, intensities = my_spec.absorption(-1500., 1600., 5., rho_g, t_final, dt, dipole_file =
                        'dipole_eet_tcl2fm_tauc%0.1f_T%0.1f_split%0.1f_run%d_PD%s.dat'%(tau_c,T,split,run,PD), is_damped=True)

    #           with open('poptest_eet_redfm_PD_tauc%0.1f_T%0.1f_lamda%0.1f_split%d_PD%s.dat'%(tau_c,T,lamda,split,PD), 'w') as f:
    #               for (time, rho_site, rho_eig) in zip(times, rhos_site, rhos_eig):
    #                   f.write('%0.8f %0.8f %0.8f\n'%(time, rho_site[1,1].real, rho_site[2,2].real))


                with open('abs_test_redfm_tauc%0.1f_T%0.1f_split%0.1f_lamda%0.1f_run%d_PD%s.dat'%(tau_c,T,split,lamda,run,PD), 'w') as f:
