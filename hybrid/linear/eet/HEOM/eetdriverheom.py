...
def main(tau_c, T, lamda, L):
  ...
  # start at [1,1] for populations!
        ...
        for beta in [1./kT]:
            for lamda in [lamda]:
                for L in [L]:
                    ...
                    my_heom = heom.HEOM(my_ham, L=L, K=1)
    ...
    #               with open('coh_eet_heom_tauc%0.01f_T%0.01f_lamda%0.01f_L%d.dat'%(tau_c,T,lamda,L), 'w') as f:
    #                   for (time, rho_site, rho_eig) in zip(times, rhos_site, rhos_eig):
    #                       f.write('%0.8f %0.8f %0.8f\n'%(time, rho_site[1,2].real, rho_site[2,1].real))


                    with open('eet_heomlarger_tauc%0.01f_T%0.01f_lamda%0.01f_L%d_K1.dat'%(tau_c,T,lamda,L), 'w') as f:
