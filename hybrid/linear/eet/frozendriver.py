...
#               t_init = 0.0
                ...
                my_ham = ham.Hamiltonian(ham_sys, ham_sysbath, spec_densities, kT, hbar=hbar, sample_wigner=False)
                
                my_frozen = frozen.FrozenModes(my_ham, nmode=nmode, ntraj=ntraj)


                my_spec = spec.Spectroscopy(dipole, my_frozen)
                omegas, intensities = my_spec.absorption(-900., 1100., 0.02, rho_g, t_final, dt, dipole_file =
...
#                       f.write('%0.8f %0.8f %0.8f\n'%(time, rho_site[1,1].real, rho_site[2,2].real))
                ...
                with open('eet_frozen_omegac%0.1f_beta%0.1f.dat'%(omega_c,beta), 'w') as f:
