...
def main():
    ...
    for omega_c in [0.1,0.3]:
        for beta in [1.,3.]:
            for lamda in [1.0, 2.0]:
                t_final = 50
                dt = 0.1
                lamda = lamda
                kT = 1./beta
                ...
                for method in ['Redfield','TCL2']:
                    my_redfield = redfield.Redfield(my_ham, method=method, is_secular=False)
    ...
    #           times, rhos_site, rhos_eig = my_redfield.propagate(rho_g, t_init, t_final, dt)
                    ...
                    with open('%s_omegac%0.1f_beta%0.1f_lamda%0.1f.dat'%(method,omega_c,beta,lamda), 'w') as f:
