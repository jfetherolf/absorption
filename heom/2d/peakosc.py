...
def main():
    ...
    for K in [1]:
        for L in [4]:
            ...
            with open('DP1_HEOM_dt-%0.0f_tf-%0.0f_L-%d_K-%d.dat'%(dt,t_final,L,K), 'w') as f:
                for t2, spectrum in zip(t2s, spectra):
                    f.write('%0.8f %0.8f\n'%(t2, spectrum[25,24]))
                    f.write('\n')
            
            with open('DP2_HEOM_dt-%0.0f_tf-%0.0f_L-%d_K-%d.dat'%(dt,t_final,L,K), 'w') as f:
                for t2, spectrum in zip(t2s, spectra):
                    f.write('%0.8f %0.8f\n'%(t2, spectrum[50,49]))
                    f.write('\n')
            
            with open('CP21_HEOM_dt-%0.0f_tf-%0.0f_L-%d_K-%d.dat'%(dt,t_final,L,K), 'w') as f:
                for t2, spectrum in zip(t2s, spectra):
                    f.write('%0.8f %0.8f\n'%(t2, spectrum[26,49]))
                    f.write('\n')

            with open('CP12_HEOM_dt-%0.0f_tf-%0.0f_L-%d_K-%d.dat'%(dt,t_final,L,K), 'w') as f:
                for t2, spectrum in zip(t2s, spectra):
                    f.write('%0.8f %0.8f\n'%(t2, spectrum[50,26]))
                    f.write('\n')
